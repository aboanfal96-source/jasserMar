...
async function fetchContractsFor(sym){
  ...
  try{
    ...
    const rsp=await fetch('/api/recommend',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({symbol:sym,technicalScore:adj,direction,weights:AI.weights,expectedMove:sc.expectedMovePct,underlyingPrice:p,historicalVol:hv})});
...
async function buildContract(){
  ...
  const synthBadge=data.data.dataSource==='synthetic'?`<div style="background:rgba(245,200,66,.12);border:1px solid rgba(245,200,66,.4);border-radius:6px;padding:8px 10px;margin-bottom:10px;font-size:10px;color:var(--gd);text-align:center;line-height:1.5">⚠️ Yahoo حظر سلسلة العقود — هذه تقديرات تركيبية بصيغة Black-Scholes للأغراض التعليمية فقط</div>`:'';
  let h=synthBadge;
  top.forEach((c,idx)=>{
